...
const DEFAULT_RESTRICTED_PATHS = ['metrics', 'favicon.ico'];
...
function observe(method, path, statusCode, start, restrictedPaths = []) {
  path = path ? path.toLowerCase() : ''
  restrictedPaths = restrictedPaths.concat(DEFAULT_RESTRICTED_PATHS)

  if (indexOf(restrictedPaths, trim(path, '/')) == -1) {
